...
function parseYamlContent($content)
{
    try {
        ...
        throw new ParseException("YAML 解析失败: " . $exception->getMessage());
    }
    return false;
}
...
function analysisFile($file)
{
    try {
        ...
        $yamlDB = parseYamlContent(str_replace(array('<pre style="word-wrap: break-word; white-space: pre-wrap;">', '</pre>', '!<str> '), "", $content));
        if (is_array($yamlDB)) {
